...
- **Validação de tipo ao adicionar**: ao incluir um alvo, o servidor sonda `host:porta` nos dois tipos (HTTP/S e TCP). Se o tipo escolhido não responde mas o outro sim, pede confirmação (`409` + sugestão) antes de salvar — ex.: escolher HTTPS numa porta SSH oferece salvar como TCP. Se nenhum tipo responde, salva assim mesmo (offline). Depois de salvo, a checagem periódica usa só o tipo confirmado, sem re-validar
